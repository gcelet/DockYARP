...
| `ssl_verify_client=optional` passthrough + CRL + global CA | ✅ | A verified client cert is forwarded to the backend as `X-SSL-Client-Verify: SUCCESS` + `X-SSL-Client-S-DN`/`I-DN` (client-supplied values stripped). `optional` is genuinely non-blocking — the handshake is per-host (`required` strict / `optional` permissive / `none` not prompted), an untrusted/revoked cert never drops the connection, and the backend receives `FAILED`/`NONE` to decide for itself. `Tls:ClientCrlPath` (global, matching the existing global-only CA) rejects a revoked certificate alongside the CA-chain check. Proven end-to-end against a real TLS handshake, not just unit-tested (`add-mtls-optional-passthrough`, `add-mtls-optional-crl`). |
...
| PROXY protocol (`ENABLE_PROXY_PROTOCOL`) + real client IP | ✅ | `Server:EnableProxyProtocol` accepts a PROXY v1/v2 header on the edge listeners and sets the connection's remote endpoint to the real client (feeding `X-Forwarded-For`/`X-Real-IP`). Parser + connection middleware unit-tested; the live v1/v2 edge path is covered end-to-end by `ProxyProtocolTests` (a dedicated `Server:EnableProxyProtocol` instance). External ports were already done. |
...
| Host-network-mode backends | ✅ | `Docker:HostAddress` targets the host on `VIRTUAL_PORT`; skipped with a warning if unset. Live reachability proven end-to-end (`HostNetworkModeTests`, `tests/DockYarp.E2E.Tests/`) — Docker Desktop requires its "Enable host networking" beta setting, native Linux CI needs none. |
| IPv6 listeners (`ENABLE_IPV6`) + `PREFER_IPV6_NETWORK` | ✅ | Edge listens dual-stack by default (Kestrel `ListenAnyIP` on `[::]`, no toggle needed); `Docker:PreferIpv6` forwards to a backend's IPv6 address (single family, IPv4/IPv6 fallback). Live IPv6-network e2e deferred. |
| Multi-network attach / unreachable-network resilience | ✅ | `Docker:ProxyNetworks` → reachability-aware selection; unreachable backends skipped. **Auto-detected** from the proxy's own networks when unset (`add-network-self-detection`, self-inspection via `HOSTNAME`). Live skip-unreachable check proven end-to-end (`MultiNetworkTests`, `tests/DockYarp.E2E.Tests/`). |
...
| Remote daemon over TLS (`DOCKER_HOST`+`DOCKER_TLS_VERIFY`/`CERT_PATH`) | ✅ | `Docker:CertPath` (dir with `ca.pem`/`cert.pem`/`key.pem`) presents a client cert to a `tcp://` daemon; `Docker:TlsVerify` validates the daemon against `ca.pem` (custom root trust). The pinned Docker.DotNet has no `CertificateCredentials`, so a custom `Credentials` wires the cert + callback onto its `ManagedHandler`. Construction/verify unit-tested; live TLS connection runtime. |
